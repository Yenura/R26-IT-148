...
export default function LoadingState({ title = "Analyzing Resume with AI...", stageDurationMs = 700 }) {
  ...
  return (
    <div style={{ padding: 48, textAlign: 'center', background: 'var(--bg-elevated)', borderRadius: 14, border: '1px solid var(--border)', margin: '20px 0' }}>
      ...
      <h3 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text)', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        <Sparkles size={18} style={{ color: 'var(--accent)' }} /> {title}
      </h3>
      <div style={{ fontSize: 14, color: 'var(--accent)', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>

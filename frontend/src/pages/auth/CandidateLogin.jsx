    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ width: 420, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800 }}>Candidate Login</h1>
          <p className="muted" style={{ fontSize: 13 }}>Sign in to your candidate account</p>
        </div>
        <form onSubmit={login} className="card" style={{ padding: 28 }}>
          <label>Email</label>
          <div style={{ position: 'relative' }}>
            <Mail size={16} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="candidate@example.com" style={{ paddingLeft: 36 }} />
          </div>
          ...
          <button className="btn" type="submit" disabled={busy} style={{ width: '100%', marginTop: 16 }}>
            <User size={16} /> {busy ? 'Signing in…' : 'Candidate Login'}
          </button>

...
def demographic_parity(df, score_col="CSS"):
    df = df.copy()
    df["sl"] = (df[score_col] >= THRESHOLD).astype(int)
    rates = {g: float(df[df[PROTECTED] == g]["sl"].mean()) for g in df[PROTECTED].unique() if len(df[df[PROTECTED] == g]) > 0}
...
def equal_opportunity(df, score_col="CSS", label_col="relevance_label"):
    ...
    if len(qdf) == 0:
        return {"EOD": 0.0, "violation": False, "rates": {}}
    tpr = {g: float(qdf[qdf[PROTECTED] == g]["sl"].mean()) for g in qdf[PROTECTED].unique() if len(qdf[qdf[PROTECTED] == g]) > 0}
...
def fair_reranking(df, score_col="CSS", top_k=20, min_prop=0.40):
    ...
    for pos in range(1, top_k + 1):
        ...
        if need_u - cur_u >= remain and u_i < len(unpr_pool):
            ranked.append(unpr_pool.iloc[u_i])
            u_i += 1
        else:
            ...
            if p_i < len(priv_pool) and ps >= us:
                ranked.append(priv_pool.iloc[p_i])
                p_i += 1
...
def print_audit(report, role_name="Overall"):
    dp = report["dp"]
    eo = report["eo"]
    status = "PASS (Fair)" if report["fair"] else "WARN (Unfair)"

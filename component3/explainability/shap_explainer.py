...
def plot_waterfall(row, cid, css, save_path=None):
    ...
    for v in svals:
        bots.append(run)
        hts.append(v)
        cols.append(COLORS["pos"] if v >= 0 else COLORS["neg"])
        run += v
    ...
    ax.axvline(phi_0, color="#7f8c8d", linestyle="--", linewidth=1.5,
               label=f"Base (phi_0) = {phi_0:.3f}")
    ax.axvline(css, color=COLORS["line"], linestyle="-", linewidth=2.5,
               label=f"CSS = {css:.3f}")
    ax.set_yticks(range(len(slbls)))
    ax.set_yticklabels(slbls, fontsize=10)
    ax.set_xlabel("SHAP Contribution (phi)", fontsize=11)
    ax.set_title(f"Candidate {cid} | CSS(c) = phi_0 + sum(phi_i) = {phi_0:.3f} + "
...
if __name__ == "__main__":
    ...
    for role in test_df["job_role"].unique():
        ...
        print(f"  {dn:<35} phi_0={shap_df['phi_0'].iloc[0]:.4f}  err={err:.2e}")

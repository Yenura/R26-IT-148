...
for p in [DATASETS,RESULTS,MODELS,CHARTS]: os.makedirs(p, exist_ok=True)
...
if __name__ == "__main__":
    ...
    print("  Interview-Driven Candidate Ranking | 10 IT Roles")
    print("  IT22027610 | Perera K.G.S.N | R26-IT-148")
    print("="*65)
    t0 = time.time()

    # ── STEP 1: Data Generation ──────────────────────────────────
    step(1, "Synthetic Dataset Generation (10 Roles × 600 = 6,000)")
    from data.generate_data import (
        generate_candidates, generate_fairness_dataset,
        generate_job_requirements, RECORDS_PER_ROLE
    )
    from data.role_configs import ROLES, ROLE_DISPLAY_NAMES

    all_dfs = [generate_candidates(r, RECORDS_PER_ROLE) for r in ROLES]
    full_df = pd.concat(all_dfs, ignore_index=True)

    # Per-role files
    for role in ROLES:
        full_df[full_df["job_role"]==role].to_csv(
            f"{DATASETS}/role_{role}.csv", index=False)

    # Train/Val/Test split
    tr_l, va_l, te_l = [], [], []
    for role in ROLES:
        rdf = full_df[full_df["job_role"]==role].sample(frac=1, random_state=42)
        n = len(rdf); t1,t2 = int(n*0.60), int(n*0.75)
        tr_l.append(rdf.iloc[:t1]); va_l.append(rdf.iloc[t1:t2]); te_l.append(rdf.iloc[t2:])
    ...
    print(f"  ✓ Full:{len(full_df):,} | Train:{len(train_df):,} | "
    ...
    s   = eng.score_one(f)
    print(f"  ✓ Test candidate: CSS={s.CSS} | Passed={s.passed_hard_filter}")

    # ── STEP 3: LambdaMART + Ablation ───────────────────────────
    step(3, "LambdaMART Training + Ablation Study")
    ...
    print("\n  Ablation:"); print(abl.to_string(index=False))
    ...
    # ── STEP 5: Fairness ────────────────────────────────────────
    step(5, "Fairness Audit (Equations 9 & 10) — All 10 Roles")
    ...
    for role, rpt in per_role_rep.items():
        ...
        rows_f.append({"Role":ROLE_DISPLAY_NAMES.get(role,role),
                       "DP":rpt["dp"]["DP"],"EOD":rpt["eo"]["EOD"],
                       "Fair":rpt["fair"]})
    overall_rpt = run_audit(fair_df)
    print("  "+"─"*58)
    print_audit(overall_rpt, "OVERALL")
    rows_f.append({"Role":"OVERALL","DP":overall_rpt["dp"]["DP"],
                   "EOD":overall_rpt["eo"]["EOD"],"Fair":overall_rpt["fair"]})
    pd.DataFrame(rows_f).to_csv(f"{RESULTS}/fairness_report.csv", index=False)

    # ── STEP 6: SHAP ────────────────────────────────────────────
    step(6, "SHAP Explainability (Equation 11) — All 10 Roles")
    ...
    for role in ROLES:
        ...
        print(f"  ✓ {ROLE_DISPLAY_NAMES[role]}")
...
{'='*65}
  RESULTS SUMMARY
{'='*65}
  Dataset       : {len(full_df):,} candidates | 10 IT roles
  Train/Val/Test: {len(train_df):,} / {len(val_df):,} / {len(test_df):,}
  Fairness set  : {len(fair_df):,} (2,500M + 2,500F)
{'─'*65}

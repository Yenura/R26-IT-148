...
<style>
    ...
    .top-header p { margin: 8px 0 0 0; opacity: 0.92; font-size: 15.5px; }
    ...
    .kpi-number { font-size: 38px; font-weight: 700; color: #1e3a5f; }
    .kpi-label { font-size: 13.5px; color: #555; margin-top: 6px; font-weight: 500; }
    ...
    /* Badges */
    .badge-excellent { background:#e8f5e9; color:#2e7d32; padding:5px 14px; border-radius:20px; font-weight:600; font-size:13px; border:1px solid #a5d6a7; }
    .badge-good      { background:#e3f2fd; color:#1565c0; padding:5px 14px; border-radius:20px; font-weight:600; font-size:13px; border:1px solid #90caf9; }
    .badge-average   { background:#fff8e1; color:#f57f17; padding:5px 14px; border-radius:20px; font-weight:600; font-size:13px; border:1px solid #ffe082; }
    .badge-weak      { background:#fce4ec; color:#c62828; padding:5px 14px; border-radius:20px; font-weight:600; font-size:13px; border:1px solid #ef9a9a; }

    /* Pills */
    .strength-pill, .weakness-pill, .neutral-pill {
        padding: 6px 14px;
        border-radius: 20px;
        font-size: 13px;
        font-weight: 500;
    ...
    .strength-pill { background:#e8f5e9; color:#2e7d32; border:1px solid #a5d6a7; }
    .weakness-pill { background:#fce4ec; color:#c62828; border:1px solid #ef9a9a; }
    .neutral-pill  { background:#f1f3f5; color:#374151; border:1px solid #d1d5db; }
    ...
    /* ====== CRITICAL FIX: MAKE SIDEBAR TOGGLE ALWAYS VISIBLE ====== */
    button[kind="icon"] {
    ...
    /* Hide Streamlit branding but keep header area (needed for toggle) */
...
def edu_label(lv):
    return {1:"Diploma", 2:"Bachelor's Degree", 3:"Master's Degree", 4:"PhD"}.get(int(lv), "Unknown")
...
def draw_radar(areas):
    ...
    vals = [pct(v) for v in areas.values()]
    vals += vals[:1]
...
def sidebar():
    with st.sidebar:
        ...
        <div style='padding:20px 0 12px 0;'>
            <div style='color:white; font-size:26px; font-weight:700;'>🎯 TalentRank</div>
            <div style='color:#90caf9; font-size:13.5px;'>Smart Recruitment Platform</div>
        </div>
        <hr style='border-color:#2d6a9f; margin:16px 0;'>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style='background:#16325a; border-radius:10px; padding:14px; margin:12px 0;'>
            <div style='color:#90caf9; font-size:12px; margin-bottom:6px;'>REQUIRED SKILLS</div>
            <div style='color:white; font-size:13.2px; line-height:1.4;'>{ROLE_REQUIRED_SKILLS[role]}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        min_exp = st.slider(
            ...
            0.0, 10.0,
            float(ROLE_REQUIREMENTS[role]["min_exp"]),
            0.5
        )
        ...
        st.markdown("---")
        st.markdown(
            "<p style='color:#90caf9; font-size:11px;'>Powered by AI Ranking Engine<br>R26-IT-148 | SLIIT</p>",
            unsafe_allow_html=True
        )
...
def tab_shortlist(full_df, role, job, top_n):
    ...
    if len(top) > 0:
        ...
        ax.set_title(f"Candidate Rankings — {dn}", fontsize=13, fontweight="bold", color="#1e3a5f", pad=12)
    ...
    for _, row in top.iterrows():
        ...
        with st.expander(f"{ric}  Candidate {cid}   |   Match Score: {pct(css)}%   |   {vd}", expanded=(rank <= 3)):
            ...
            with right:
                areas = {
                    "Education": float(row.get("S_edu", 0)),
                    "Experience": float(row.get("S_exp", 0)),
                    "Skills": float(row.get("S_skill", 0)),
                    "Knowledge": float(row.get("P_mcq", 0)),
                    "Problem\nSolving": float(row.get("P_desc", 0)),
                    "Coding": float(row.get("P_code", 0))
                }
                st.pyplot(draw_radar(areas), use_container_width=True)
                plt.close()

                vc = match_color(css)
                vbg = match_bg(css)
                st.markdown(f"""
                <div style='background:{vbg};border:1px solid {vc};border-radius:10px;padding:14px;text-align:center;margin-top:12px;'>
                    <div style='font-size:13px;color:#666;margin-bottom:4px;'>Overall Assessment</div>
                    <div style='font-size:28px;font-weight:700;color:{vc};'>{pct(css)}%</div>
                    <div style='font-size:13px;font-weight:600;color:{vc};margin-top:4px;'>{vd}</div>
                    <div style='font-size:13px;margin-top:6px;'>{stars(css)}</div>
                </div>
                """, unsafe_allow_html=True)
    ...
    if len(failed) > 0:
        ...
        with st.expander(f"⛔  {len(failed)} candidates did not meet the minimum requirements"):
            ...
            for _, row in failed.head(20).iterrows():
                r = (
                    row.get("filter_fail_reason", "")
                    .replace("edu_level", "education level")
                    .replace("min_edu", "minimum required")
                    .replace("S_skill", "skill level")
                    .replace("P_code", "coding ability")
                    .replace("threshold", "minimum")
                )
...
def tab_insights(full_df, role, job, top_n, passed, role_df):
    ...
    if len(passed) == 0:
        st.warning("No qualified candidates found.")
        return
    ...
    int_w = ROLE_INTERVIEW_WEIGHTS[role]
    importance = {
        "Technical Skills": cv_w["w_skill"] * 0.40,
        "Practical Coding": int_w["w_code"] * 0.60,
        "Problem Solving": int_w["w_desc"] * 0.60,
        "Knowledge Test": int_w["w_mcq"] * 0.60,
        "Work Experience": cv_w["w_exp"] * 0.40,
        "Education": cv_w["w_edu"] * 0.40,
    }
    ...
    fig.patch.set_facecolor("white")
    ax.set_facecolor("#f8f9fa")
    lbls = list(importance.keys())
    vals = [v * 100 for v in importance.values()]
    ...
    base = {
        "Education": passed["S_edu"].mean(),
        "Work Experience": passed["S_exp"].mean(),
        "Technical Skills": passed["S_skill"].mean(),
        "Knowledge Test": passed["P_mcq"].mean(),
        "Problem Solving": passed["P_desc"].mean(),
        "Practical Coding": passed["P_code"].mean(),
    }
    ...
    for _, row in top.iterrows():
        ...
        above = [(k, v) for k, v in cscores.items() if v > base[k] + 0.05]
        above.sort(key=lambda x: x[1], reverse=True)
        below = [(k, v) for k, v in cscores.items() if v < base[k] - 0.05]
        ...
        with st.expander(f"#{rank} — {cid} | {pct(css)}% Match | {vd}"):
            ...
            with ca:
                ...
                if above:
                    for area, val in above[:3]:
                        ...
                        st.markdown(
                            f'<span class="strength-pill">✓ {area}: {pct(val)}% (+{ab}% vs average)</span>',
                            unsafe_allow_html=True,
                        )
                else:
            ...
            with cb:
                ...
                if below:
                    for area, val in below[:3]:
                        ...
                        st.markdown(
                            f'<span class="weakness-pill">⚠ {area}: {pct(val)}% (-{bel}% vs average)</span>',
                            unsafe_allow_html=True,
                        )
                else:
            ...
            al = list(cscores.keys())
            cv = [pct(cscores[a]) for a in al]
            av = [pct(base[a]) for a in al]
            x = np.arange(len(al))
            ...
            for bar in b1:
                ax.text(
                    ...
                    ha="center", va="bottom",
                    fontsize=8, color="#1e3a5f", fontweight="bold"
                )
            plt.tight_layout()
            st.pyplot(fig, use_container_width=True)
            plt.close()
            ...
            tips = {
                "Practical Coding": "Include a live coding exercise to verify programming ability.",
                "Problem Solving": "Ask scenario-based questions to assess how they approach complex problems.",
                "Knowledge Test": "Ask technical theory questions to verify domain knowledge.",
                "Technical Skills": "Discuss specific technologies on the job description in depth.",
                "Work Experience": "Ask them to describe past projects in detail.",
                "Education": "Ask how their academic background relates to this role.",
            }
            if below:
                ...
                st.markdown(
                    "- This candidate is strong across all areas. Focus the interview on cultural fit and specific project experience."
                )
...
def tab_comparison(full_df, role, job, top_n, passed, role_df):
    ...
    if len(passed) == 0:
        st.warning("No qualified candidates to compare.")
        return
    ...
    ax.set_xticklabels(list(sc.keys()), fontsize=10, fontweight="bold")
    ax.set_yticks(range(len(clbls)))
    ax.set_yticklabels(clbls, fontsize=9)
    for i in range(len(clbls)):
        for j in range(len(sc)):
            val = hm[i, j]
            tc = "white" if val < 40 or val > 75 else "black"
    ...
    if len(top3) > 0:
        ...
        for i, (_, row) in enumerate(top3.iterrows()):
            ...
            with cols[i]:
                st.markdown(f"""
                <div style='background:{vbg};border:1px solid {vc};border-radius:12px;padding:18px;text-align:center;'>
                    <div style='font-size:15px;font-weight:700;color:{vc};margin-bottom:8px;'>{medals[i]}</div>
                    <div style='font-size:20px;font-weight:700;color:#1e3a5f;'>{row['candidate_id']}</div>
                    <div style='font-size:28px;font-weight:700;color:{vc};margin:8px 0;'>{pct(css)}%</div>
                    <div style='font-size:13px;color:{vc};'>{verdict(css)}</div>
                    <div style='font-size:14px;margin-top:6px;'>{stars(css)}</div>
                </div>
                """, unsafe_allow_html=True)

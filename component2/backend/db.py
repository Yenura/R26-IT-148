...
def _get_mongo_db() -> Optional[Database]:
    ...
    db_name = os.getenv("MONGODB_DB", "component2_interview")
...
def get_db() -> Any:
    """Return active DB handle (MongoDB if available, otherwise in-memory shim)."""
...
def get_result(interview_id: str) -> Optional[Dict[str, Any]]:
    mongo_db = _get_mongo_db()
    if mongo_db is not None:

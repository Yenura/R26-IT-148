...
def _run_code_in_sandbox(code_text: str, inp: dict) -> str:
    ...
    sandbox_wrapper = (
        ...
        "_delattr = _builtins_mod.delattr\n"
        "_setattr = _builtins_mod.setattr\n"
        "_SAFE_BUILTINS = {\n"
        ...
        "        _delattr(_builtins_mod, _k)\n"
        "for _k, _v in _SAFE_BUILTINS.items():\n"
        "    _setattr(_builtins_mod, _k, _v)\n"
        "_setattr(_builtins_mod, '__import__', lambda name, *a, **kw: (_ for _ in ()).throw(ImportError(f'Blocked: {name}')))\n"
        ...
        "    raise ImportError(f'Blocked: {name}')\n"
        "_setattr(_builtins_mod, '__import__', _safe_import)\n"
        "_stdout = _sys.stdout\n"

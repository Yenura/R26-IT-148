...
def print_model_accuracy_banner():
    ...
    # Attempt to load model and test metrics
    ...
    if os.path.exists(test_csv_path) and os.path.exists(c3_model_path):
        try:
            import pandas as pd
            from scipy.stats import spearmanr
            df_test = pd.read_csv(test_csv_path)
            with open(c3_model_path, "rb") as f:
                model = pickle.load(f)
            
            feat_cols = ["S_edu", "S_exp", "S_skill", "P_mcq", "P_desc", "P_code"]
            if all(col in df_test.columns for col in feat_cols):
                preds = model.predict(df_test[feat_cols])
                df_test["ltr_pred"] = preds
                
                # Compute sample NDCG
                def dcg(r, k=10):
                    r = np.array(r[:k], dtype=float)
                    return np.sum((2**r - 1) / np.log2(np.arange(2, len(r) + 2))) if len(r) else 0.0

                ndcgs = []
                for _, group in df_test.groupby("job_role"):
                    sorted_g = group.sort_values("ltr_pred", ascending=False)
                    rels = sorted_g["relevance_label"].tolist() if "relevance_label" in sorted_g else sorted_g["CSS"].tolist()
                    actual_dcg = dcg(rels, 10)
                    ideal_dcg = dcg(sorted(rels, reverse=True), 10)
                    if ideal_dcg > 0:
...
def main():
    ...
    print("\n✨ All RecruitAI services are online and ready.")
